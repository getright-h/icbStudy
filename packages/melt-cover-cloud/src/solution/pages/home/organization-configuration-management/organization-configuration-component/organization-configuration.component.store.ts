...
import { IOrganizationConfigurationState, OrgData } from './organization-configuration.interface';
import {
  EquityList,
  EquityListPackage,
...
export function useOrganizationConfigurationStore() {
  const { state, setStateWrap, getState } = useStateStore(new IOrganizationConfigurationState());
  const form = useForm();
  const form1 = useForm();
  const form2 = useForm();
