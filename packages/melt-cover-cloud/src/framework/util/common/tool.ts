...
export function unParams(params = '?a=1&b=2&c=3') {
  ...
  params &&
    ...
    params.replace(/((\w*)=([\.a-z0-9A-Z%_-]*)?)?/g, (m, a, b, c): any => {
      if (b || c) obj[b] = c;
    });
  return obj;
}
...
export function getMiniprogram(): boolean {
  const ua = getBrowserNavigatorMetaInfo().toLowerCase();
...
// 处理内联样式 px to rem
export function activeSetPxToRemInLineCss(params: number | string): string {
  const unit = 16;
  if (typeof params == 'string' && !isNaN(parseInt(params))) {
    return parseInt(params) / 16 + 'rem';
  } else if (typeof params == 'number') {
    return params / unit + 'rem';
  }
  return params;
}
